...
+ (NSString *)encryptByRsaWithString:(NSString *)str publicKey:(NSString *)pubKey {
    ...
    for (NSInteger i = 0; i < ceilf(orstr.length / 117.0); i ++) {
        ...
        NSString *ss = [self encryptString:subStr publicKey:pubKey];
...
+ (NSString *)encryptByRsaWith:(NSString *)str publicKey:(NSString *)pubKey {
    NSMutableString *encryptStr = @"".mutableCopy;
    for (NSInteger i = 0; i < ceilf(str.length / 117.0); i ++) {
        NSString *subStr = [str substringWithRange:NSMakeRange(i * 117, MIN(117, str.length - i * 117))];
        NSString *ss = [self encryptString:subStr publicKey:pubKey];
        [encryptStr appendString:ss];
    }
    return encryptStr;
...
+ (NSString *)decryptByRsaWith:(NSString *)str privateKey:(NSString *)privKey {
    ...
    for (NSInteger i = 0; i < ceilf(str.length / 172); i ++) {
        ...
        NSString *rrr = [self decryptString:subStr privateKey:privKey];
